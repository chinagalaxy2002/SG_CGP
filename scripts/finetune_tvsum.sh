...
log_dir=$(python -c "from hydra import initialize, compose; initialize(config_path='./configs', version_base=None); cfg = compose(config_name='finetune_tvsum'); print(cfg.paths.log_dir)")
exp_list_name=$(python -c "from hydra import initialize, compose; initialize(config_path='./configs', version_base=None); cfg = compose(config_name='finetune_tvsum'); print(cfg.exp_list_name)")
...
do
    ...
    do
        ...
        --config-name finetune_tvsum data.dataset.domain=${domain} task_name=${task_name} seed=${seed} \

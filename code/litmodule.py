...
from code.metrics.matching.metrics import MatchingMetric
from code.metrics.metrics_collection import (
    get_aux_metrics,
...
class MomentRetrievalRunner(LightningModule):  # noqa: WPS214,WPS230,E302
    ...
    def _safe_log_dict(self, dictionary: Dict[str, Any], **kwargs: Any) -> None:
        cuda_dict = {}
        for k, v in dictionary.items():
            if isinstance(v, (int, float)):
                cuda_dict[k] = torch.tensor(v, dtype=torch.float32, device=self.device)
            elif isinstance(v, torch.Tensor):
                cuda_dict[k] = v.to(self.device)
            else:
                cuda_dict[k] = v
        self.log_dict(cuda_dict, **kwargs)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        for name, value in detr_val_metrics.items():  # noqa: WPS204
            if "HL" in name:
                self._safe_log(name, value, on_epoch=True, sync_dist=True, prog_bar=True)
            elif name[4:] in MAIN_METRICS_DICT:
                self._safe_log(name, value, on_epoch=True, sync_dist=True, prog_bar=True)
            else:
                self._safe_log(name, value, on_epoch=True, sync_dist=True)

        # log aux metrics
        for name, value in aux_val_metrics.items():
            if name[4:] in MAIN_METRICS_DICT:
                self._safe_log(f"{name}-AUX", value, on_epoch=True, sync_dist=True)

        # log comb metrics
        for name, value in comb_val_metrics.items():
            if name[4:] in MAIN_METRICS_DICT:
                self._safe_log(f"{name}-COMB", value, on_epoch=True, sync_dist=True)

        # log matching
        for name, value in valid_matching_metrics.items():
            self._safe_log(f"val/{name}", value, on_epoch=True, sync_dist=True)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        # log detr metrics
        self._safe_log_dict(detr_test_metrics, on_epoch=True, sync_dist=True)
        for name, value in matching_test_metrics.items():
            self._safe_log(f"test/{name}", value, on_epoch=True, sync_dist=True)
    ...
    def on_train_epoch_end(self) -> None:  # noqa: C901
        ...
        for name, value in train_matching_metrics.items():
            self._safe_log(f"train/{name}", value, on_epoch=True, sync_dist=True)
        ...
        if self.current_epoch % self.check_train_every_n_epoch == 0 and self.current_epoch != 0:
            ...
            for name, value in detr_train_metrics.items():
                if name[6:] in MAIN_METRICS_DICT:
                    self._safe_log(f"{name}", value, on_epoch=True, sync_dist=True)

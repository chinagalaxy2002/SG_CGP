...
TASK_NAME="${TASK_NAME:-sg_detr_dq_cgp_ft_identity_span_safe_resume60}"
...
if [[ -z "$RESUME_CHECKPOINT" || ! -f "$RESUME_CHECKPOINT" ]]; then
    echo "[ERROR] Set RESUME_CHECKPOINT to the stage-1 epoch=036 checkpoint" >&2
    exit 1
fi

echo "Resuming identity-start, span-safe DQ-CGP fine-tuning to epoch 60"
echo "GPU: $TARGET_GPU"
echo "Resume checkpoint: $RESUME_CHECKPOINT"
echo "Task: $TASK_NAME"
echo "Fixed seed: 40 (no random-seed sweep)"
